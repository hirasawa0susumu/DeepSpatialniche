...
# Compute niche token if available
has_niche = (niche_ref_0 is not None and niche_ref_1 is not None)
if has_niche:
    _ne = model.niche_encoder
    is_ms = 'local_neighbors' in niche_ref_0
    nref = niche_ref_0
    if is_ms:
        def _gs(s):
            nbr = nref[f'{s}_neighbors'][idx]
            return {
                'g_nbr': g0[nbr], 'delta': nref[f'{s}_deltas'][idx],
                'dist': nref[f'{s}_dists'][idx], 'mask': nref[f'{s}_mask'][idx],
                'c_nbr': c0[nbr],
            }
        sl, sm, sg = _gs('local'), _gs('mid'), _gs('global')
        batch['niche_token'] = _ne(
            g_center=g0_s, pos_center=x0_s, ct_center=c0_s,
        ...
        if 'uot_g_to_next' in nref:
            gm, pm = nref['uot_g_to_next'], nref['uot_pos_to_next']
            for scale in ('local', 'mid', 'global'):
                nbr = nref[f'{scale}_neighbors'][idx]
                nn[f'g_nbr_target_{scale}'] = gm[nbr]
                nn[f'pos_nbr_target_{scale}'] = pm[nbr]
        batch['niche_nbr_data'] = nn
    else:
        nbr_idx = nref['neighbors'][idx]
        batch['niche_token'] = _ne(
            g_center=g0_s, pos_center=x0_s,
            g_nbrs=g0[nbr_idx], delta_nbrs=nref['deltas'][idx],
            dist_nbrs=nref['dists'][idx], mask_nbr=nref['mask'][idx],
        )
...
with torch.no_grad():
    vx_train, vg_train, vc_train = model.module.model(
        ...
        zt=_z_mid_t, delta_z=batch['delta_z'], ct=c_onehot, niche_token=None,
    )
...
idx1 = torch.randperm(x1.shape[0])[:n]  # separate indices for x1
...
print(f"Max displacement over trajectory: {delta:.4f} (norm), "
      f"{delta * model.spatial_stats['x_range']:.2f}μm (physical)")
